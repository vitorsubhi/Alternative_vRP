...
$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data["which"] == 27){
			$.post("http://emergency/closeSystem");
		};
	};
});
/* ---------------------------------------------------------------------------------------------------------------- */
...
const functionSearch = (passaporte) => {
	if (passaporte != ""){
		$.post("http://emergency/searchUser",JSON.stringify({ passaporte: parseInt(passaporte) }),(data) => {
						...
						${data["result"][4].map((data) => (`
							<div class="recordBox">
								<div class="fineSeachTitle">
									<span style="width: 250px; float: left;"><b>Policial:</b> ${data["emergency"]}</span>
...
$(document).on("click","#portSearch",function(e){
	$.post("http://emergency/updatePort",JSON.stringify({ passaporte: e["target"]["dataset"]["id"] }));
});
/* ----------CLICKBUY---------- */
$(document).on("click","#penalSearch",function(e){
	$.post("http://emergency/updatePenal",JSON.stringify({ passaporte: e["target"]["dataset"]["id"] }));
});
/* ---------------------------------------------------------------------------------------------------------------- */
...
const functionCalculadora = () => {
	selectPage = "Calculadora";

	$('#content').html(`
	<div id="titleContent">CALCULADORA</div><iframe src="http://yeahrp.com/policia" width="100%" height="1000px" frameBorder="0"></iframe>`);
};
/* ----------BUTTONPRISON---------- */
$(document).on("click",".buttonPrison",function(e){
	...
	if (passaporte != "" && servicos != "" && multas != "" && texto != ""){
		$.post("http://emergency/initPrison",JSON.stringify({
...
$(document).on("click",".buttonFine",function(e){
	...
	if (passaporte != "" != "" && multas != "" && texto != ""){
		$.post("http://emergency/initFine",JSON.stringify({
			passaporte: parseInt(passaporte),
			multas: parseInt(multas),
			texto: texto
		}));
