...
Citizen.CreateThread(function()
	while true do
		...
		if Distance < 6.1 and not IsPedInAnyVehicle(ped) then
			...
			local distance_f = #((GetEntityCoords(closestVehicle) + GetEntityForwardVector(closestVehicle) * -1) - GetEntityCoords(ped))
			local distance_b = #((GetEntityCoords(closestVehicle) + GetEntityForwardVector(closestVehicle)) - GetEntityCoords(ped))
			if distance_b > distance_f  then
...
Citizen.CreateThread(function()
	while true do
		...
		if Vehicle.Vehicle ~= nil then
			local ped = PlayerPedId()
			if IsControlPressed(0,244) and GetEntityHealth(ped) > 100 and IsVehicleSeatFree(Vehicle.Vehicle,-1) and GetEntityHealth(ped) > 101 and not GetVehicleDoorsLockedForPlayer(Vehicle.Vehicle) and not IsEntityAttachedToEntity(ped,Vehicle.Vehicle) and not (GetEntityRoll(Vehicle.Vehicle) > 75.0 or GetEntityRoll(Vehicle.Vehicle) < -75.0) then
				...
				while true do
					...
					if Vehicle.IsInFront then
						SetVehicleForwardSpeed(Vehicle.Vehicle,-2.0)
					else
						SetVehicleForwardSpeed(Vehicle.Vehicle,2.0)
					end
