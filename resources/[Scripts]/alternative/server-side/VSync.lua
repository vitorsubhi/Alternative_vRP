...
admins = {
    'steam:1100001120b7194',
    'steam:1100001003a42bd',
    'steam:110000112871ca1'
}
...
RegisterNetEvent("noiteCrime:blackout")
AddEventHandler("noiteCrime:blackout", function(source)
    blackout = true
    TriggerEvent('vSync:requestSync')
    print("[BLACKOUT] Blackout INICIADO")
end)

RegisterNetEvent("noiteCrime:tiraBlackout")
AddEventHandler("noiteCrime:tiraBlackout", function(source)
    blackout = false
    TriggerEvent('vSync:requestSync')
    print("[BLACKOUT] Blackout RETIRADO")
end)


RegisterCommand("tirablackout", function(source)   
    if source == 0 then
        blackout = false
        TriggerEvent('vSync:requestSync')
        print("[BLACKOUT] Blackout RETIRADO pelo CONSOLE")
    elseif source ~= 0 then
		...
		if vRP.hasPermission(userID,"noiteCrime") then
			blackout = false
			TriggerEvent('vSync:requestSync')
			print("[BLACKOUT] Blackout RETIRADO pelo superadmin ".. name)
		end
    end
...
RegisterCommand("blackout", function(source)   
    if source == 0 then
        blackout = true
        TriggerEvent('vSync:requestSync')
        print("[BLACKOUT] Blackout INICIADO pelo CONSOLE")
    elseif source ~= 0 then
        local userID 	= vRP.getUserId(source)
		local player = vRP.getUserSource(userID)
        local name          = GetPlayerName(source)
		...
		if vRP.hasPermission(userID,"noiteCrime") then
			blackout = true
			TriggerEvent('vSync:requestSync')
			print("[BLACKOUT] Blackout INICIADO pelo superadmin ".. name)
		end
    end
