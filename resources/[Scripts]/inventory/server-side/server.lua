...
function cRP.dropItem(nameItem,slot,amount,x,y,z,gridZone)
	...
	if user_id then
		if activeItens[user_id] == nil and not vPLAYER.getHandcuff(source) and not vRP.wantedReturn(user_id) then
			if noFull[nameItem] or ((noCop[splitString(nameItem,"_")[1]] or noCop[nameItem]) and vRP.hasPermission(user_id,"Police")) then
				TriggerClientEvent("inventory:Update",source,"updateMochila")
...
AddEventHandler("inventory:sendItem",function(itemName,amount)
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if ((noCop[splitString(itemName,"_")[1]] or noCop[itemName] or noCop[splitString(itemName,"-")[1]]) and vRP.hasPermission(user_id,"Police")) then
			TriggerClientEvent("homes:Update",source,"requestChest")
			TriggerClientEvent("Notify",source,"amarelo","Armazenamento proibido.",5000)
			return
		end
		if activeItens[user_id] == nil and not vPLAYER.getHandcuff(source) then
			...
			if not vRP.wantedReturn(user_id) then
				...
				if nplayer then
					...
					if nuser_id then
						if (vRP.inventoryWeight(nuser_id) + (itemWeight(itemName) * parseInt(amount))) <= vRP.getBackpack(nuser_id) then
							if vRP.itemSubTypeList(itemName) then
								if (vRP.getInventoryItemAmount(nuser_id,itemName) + amount) > vRP.itemMaxAmount(itemName) then
									TriggerClientEvent("Notify",source,"vermelho","O cidadão já possúi um item do mesmo ou atingiu o limite.",5000)
								else
									...
									if vRP.tryGetInventoryItem(user_id,itemName,amount,true) then
										...
										vRP.Log("```prolog\n[ID]: "..user_id.."\n[PARA]: "..nuser_id.."\n[ENVIOU]: "..itemName.."\n[QUANTIDADE]: "..amount.."\n[Durabilidade]: "..durability.." "..os.date("\n[DATA]: %d/%m/%Y [HORA]: %H:%M:%S").." \r```", "enviou")
									end
								end
							else
		...
		if activeItens[user_id] == nil and not vCLIENT.enterVehicle(source) then
			...
			if itemType(totalName) == "Armamento" then
				...
				if checkWeapon then
                    ...
                    if ammoPlayers[user_id] == nil then
                        ammoPlayers[user_id] = {}
                    end
			...
			elseif itemType(totalName) == "Munição" then
				...
				return
			elseif itemType(totalName) == "Consumable" then
				useConsumable(source, totalName, slot)
				return
			elseif itemType(totalName) == "Usável" or itemType(totalName) == "Animal" or itemType(totalName) == "Comum" then
				...
				if nameItem == "hat" then
					TriggerClientEvent("nation_skinshop:setHat",source)
				return end

				if nameItem == "mask" then
					TriggerClientEvent("nation_skinshop:setMask",source)
				return end

				if nameItem == "gloves" then
					TriggerClientEvent("nation_skinshop:setArms",source)
				return end

				if nameItem == "glasses" then
					TriggerClientEvent("nation_skinshop:setGlasses",source)
				return end
    ...
    if user_id then
        if args[1] then
            if vRP.hasPermission(user_id,"Owner") then
				vCLIENT.putWeaponHands(source,args[1],parseInt(args[2]))
			end
        end
    end
end)
...
RegisterCommand('pet',function(source,args,rawCommand)
	if args[2] then
		local usource = vRP.userSource(args[1])
		TriggerClientEvent("dynamic:animalSpawn",usource,args[2])
	else
		TriggerClientEvent("dynamic:animalSpawn",source,args[1])
	end
end)
