...
const updateDrag = () => {
	...
	$(".empty").droppable({
		...
		drop: function(event,ui){
			...
			} else if (origin === "invRight" && tInv === "invLeft" && type == "chest"){
				$.post("http://inventory/takeItem",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
				}));
			} else if (origin === "invLeft" && tInv === "invRight" && type == "chest"){
			...
			} else if (origin === "invRight" && tInv === "invRight" && type == "chest"){
				$.post("http://inventory/updateChest",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
			...
			} else if (origin === "invRight" && type == "crafting"){
				itemData = { key: ui.draggable.data("item-key") };
				const target = $(this).data("slot");

				if (itemData.key === undefined || target === undefined) return;

				$.post("http://inventory/functionCraft",JSON.stringify({
					craft: selectCraft,
					index: itemData.key,
					slot: target,
					amount: parseInt($(".amount").val())
...
const updateMochila = () => {
	...
	$.post("http://inventory/request"+type,JSON.stringify({ craft: selectCraft }),(data) => {
		...
		var rightInventory;
		if (data["myChest"]){
			rightInventory = data["myChest"]
		}
		else if (data["drop"]){
			rightInventory = data["drop"].sort((a,b) => (a["name"] > b["name"]) ? 1:-1);
		}
		...
		for (let x = 1; x <= 20; x++){
			...
			if (rightInventory[x - 1] !== undefined){
				const v = rightInventory[x - 1];
				const maxDurability = 86400 * v["days"];
				const newDurability = (maxDurability - v["durability"]) / maxDurability;
				var actualPercent = newDurability * 100;
				...
				const item = `<div class="item populated" style="background-image: url('http://yeahrp.com/items/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-item-key="${v["key"]}" data-id="${v["id"]}" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-slot="${slot}">
					<div class="top">
						<div class="itemWeight">${(v["peso"] * v["amount"]).toFixed(2)}</div>
						<div class="itemAmount">${formatarNumero(v["amount"])}x</div>
					</div>
