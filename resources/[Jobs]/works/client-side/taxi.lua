...
-----------------------------------------------------------------------------------------------------------------------------------------
taxiRP = {}
Tunnel.bindInterface("works",taxiRP)
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if distance <= 2 then
				...
				if inService then
					...
					DrawText3D(initService[1],initService[2],initService[3],"~g~E~w~   TAXISTA")
				end
				...
				if IsControlJustPressed(1,38) then
					if inService then
						if vSERVER.initService(false) then
							inService = false
							TriggerEvent("Notify","amarelo","Serviço de <b>Taxista</b> foi finalizado.",5000)
						...
						if vSERVER.initService(true) then
							...
							currentStatus = false
							inService = true
							TriggerEvent("Notify","amarelo","Serviço de <b>Taxista</b> foi iniciado.",5000)
							lastPassenger = nil
							blipPassenger()
						end
					end
